...
def calculate_safety(data):
    ...
    if good_ratio == 1:  # Perfect driving
        ...
        # Major accident
        if data['actions'].apply(lambda x: ast.literal_eval(x)[4]).sum() > 0:
            return ("F", "red")
        # Minor accident
        elif data['actions'].apply(lambda x: ast.literal_eval(x)[3]).sum() > 0:
            return ("D", "#EEC700")
        # Major violation
        elif data['actions'].apply(lambda x: ast.literal_eval(x)[2]).sum() > 0:
            return ("C", "#EEC700")
        else:  # Minor violation
            minor = data['actions'].apply(
                lambda x: ast.literal_eval(x)[1]).sum()
...
def plot_trials(csv):
    ...
    data['average_reward'] = (data['net_reward'] / (data['initial_deadline'] -
                                                    data['final_deadline'])).rolling(window=10, center=False).mean()
    # compute avg. net reward with window=10
    data['reliability_rate'] = (
        data['success']*100).rolling(window=10, center=False).mean()
    data['good_actions'] = data['actions'].apply(
    ...
    if csv != 'sim_no-learning.csv':
        ...
        ax.plot(training_data['trial'], training_data['epsilon'],
                color='blue', label='Exploration factor')
        ax.plot(training_data['trial'], training_data['alpha'],
                color='green', label='Learning factor')

        ax.legend(bbox_to_anchor=(0.5, 1.19), fancybox=True,
    ...
    actions = training_data[['trial', 'good', 'minor',
                             'major', 'minor_acc', 'major_acc']].dropna()
    ...
    ax.plot(actions['trial'], (1 - actions['good']), color='black',
    ...
    ax.plot(actions['trial'], actions['major_acc'],
            color='red', label='Major Accident', linewidth=2)
    ...
    if len(testing_data) > 0:
        ...
        reliability_rating, reliability_color = calculate_reliability(
            testing_data)

        # Write success rate
        ax.text(0.40, .9, "{} testing trials simulated.".format(
            len(testing_data)), fontsize=14, ha='center')
        ax.text(0.40, 0.7, "Safety Rating:", fontsize=16, ha='center')
        ax.text(0.40, 0.42, "{}".format(safety_rating),
                fontsize=40, ha='center', color=safety_color)
        ax.text(0.40, 0.27, "Reliability Rating:", fontsize=16, ha='center')
        ax.text(0.40, 0, "{}".format(reliability_rating),
